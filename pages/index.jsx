import { useState, useEffect } from 'react'; import FeedList from '@/components/FeedList';

const BACKEND_URL = 'https://crisiswatch-api-production.up.railway.app/';

export default function DashboardPage() { const [threats, setThreats] = useState([]); const [loadingThreats, setLoadingThreats] = useState(true); const [query, setQuery] = useState(''); const [checking, setChecking] = useState(false); const [result, setResult] = useState(null); const [aiScore, setAiScore] = useState(null); const [scoring, setScoring] = useState(false);

useEffect(() => { const fetchThreats = async () => { setLoadingThreats(true); try { const res = await fetch(${BACKEND_URL}/api/threats); const json = await res.json(); setThreats(json.items || []); } catch (err) { console.error('Failed to fetch threats:', err); } finally { setLoadingThreats(false); } };
...
const scoreThreat = async () => { if (!query.trim()) return; setScoring(true); setAiScore(null); try { const res = await fetch(${BACKEND_URL}/api/score, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: query }), }); const data = await res.json(); setAiScore(data); } catch (err) { setAiScore({ error: 'Error scoring threat' }); } finally { setScoring(false); } };

return ( <div className="p-6 grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3"> <div className="bg-white dark:bg-gray-900 rounded-2xl shadow p-4 col-span-full"> <h1 className="text-2xl font-bold mb-4 text-black dark:text-white">Threat Feed</h1> {loadingThreats ? ( <p className="text-gray-500 dark:text-gray-400">Loading...</p> ) : threats.length === 0 ? ( <p className="text-gray-500 dark:text-gray-400">No threats found.</p> ) : ( <ul className="space-y-2"> {threats.map((item, idx) => ( <li key={idx} className="border-b border-gray-300 dark:border-gray-700 pb-2"> <a
href={item.link}
target="_blank"
rel="noopener noreferrer"
className="text-blue-600 dark:text-blue-400 hover:underline"
> {item.title} </a> </li> ))} </ul> )} </div>
